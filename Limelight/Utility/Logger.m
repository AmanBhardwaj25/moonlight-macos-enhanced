    ...
    switch (category) {
        case LoggerNoiseCategoryDiscoveryChatter:
            if ([sampleLine rangeOfString:@"Resolved address:" options:NSCaseInsensitiveSearch].location != NSNotFound) {
                summary = [NSString stringWithFormat:@"%@: %.0fs, %ld times (%@ address@address resolution repeats)",
                           NoiseCategoryDisplayName(category),
                           kNoiseAggregationWindowSec,
                           (long)count,
                           discoveryHost.length > 0 ? discoveryHost : @"unknown"];
            } else {
                summary = [NSString stringWithFormat:@"%@: %.0fs, %ld times (%@ address@, %@)",
                           NoiseCategoryDisplayName(category),
                           kNoiseAggregationWindowSec,
                           (long)count,
                           discoveryHost.length > 0 ? discoveryHost : @"unknown",
        ...
        default:
            summary = [NSString stringWithFormat:@"%@: %.0fs, %ld entries (primary reason: %@, target %@)",
                       NoiseCategoryDisplayName(category),
                       kNoiseAggregationWindowSec,
                       (long)count,
