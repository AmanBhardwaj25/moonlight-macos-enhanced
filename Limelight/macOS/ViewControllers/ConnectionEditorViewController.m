...
- (void)loadView {
    ...
    NSTextField *title = [NSTextField labelWithString:[NSString stringWithFormat:@"Connection Method - %@", self.host.displayName ?: @"-"]];
    ...
    NSTextField *defaultLabel = [NSTextField labelWithString:@"Default Connection Method"];
    ...
    NSTextField *currentLabel = [NSTextField labelWithString:@"Current Connection Method"];
    ...
    self.addField.placeholderString = @"Add address (IP/domain/IPv6, optional port)";
    ...
    self.removeButton = [NSButton buttonWithTitle:@"Delete" target:self action:@selector(removeSelectedEndpoint)];
...
- (NSString *)statusTextForAddress:(NSString *)addr {
    ...
    if (effectiveState == 1) {
        if (latency && latency.intValue >= 0) {
            return [NSString stringWithFormat:@"Online (%dms)", latency.intValue];
        }
        return @"Online";
    }
    if (effectiveState == 0) {
...
- (void)handleDoubleClick:(id)sender {
    ...
    alert.messageText = @"Edit Connection Method";
    alert.informativeText = @"Changing the address will replace the current connection method";
