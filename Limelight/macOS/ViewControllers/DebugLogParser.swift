...
enum DebugLogParser {
  ...
  private static func defaultPresentation(
    ...
    case "discovery.mdns":
      if normalized == "starting discovery" {
        return .init(title: "Start host scan", detail: "Preparing to probe known hosts and local network broadcast services")
      }
      if normalized == "starting mdns discovery" {
        return .init(title: "Start mDNS discovery", detail: nil)
      }
      if normalized == "stopping discovery" {
        return .init(title: "Stop host scan", detail: nil)
      }
      if normalized == "stopping mdns discovery" {
        return .init(title: "Stop mDNS discovery", detail: nil)
      }
      ...
      if let host = firstCapture(in: message, pattern: #"Found existing host through MDNS:\s*(.+)$"#) {
        return .init(title: "mDNS discovered known host", detail: host)
      }
    ...
    case "pairing.identity":
      ...
      if normalized.contains("client certificate imported in memory without keychain access") {
        return .init(title: "Client certificate imported into memory", detail: "This connection will not write to the keychain")
      }
      if let captures = captures(in: message, pattern: #"incorrect host:\s*([^\s]+)\s+expected:\s*([^\s]+)"#), captures.count >= 2 {
        ...
        return .init(title: "Received host response with identity mismatch", detail: "actual \(actual) · expected \(expected)")
      }
      return .init(title: cleaned, detail: nil)
    ...
    case "network.http", "network.transport":
      ...
      if normalized.contains("app list successfully retreived") || normalized.contains("app list successfully retrieved") {
        ...
        return .init(title: "App list fetched successfully", detail: "retry count \(tries)")
      }
    ...
    case "stream.lifecycle":
      if normalized.contains("stream target selection:") {
        ...
        return .init(title: "Selected stream target", detail: active)
      }
      if normalized.contains("stream target classification:") {
        let reason = value(forKey: "reason", in: message)
        return .init(title: "Completed stream route classification", detail: reason)
      }
      if let detail = suffix(in: cleaned, after: "Stream risk assessment:") {
        return .init(title: "Stream profile assessment", detail: detail)
      }
      if let detail = suffix(in: cleaned, after: "Recommended fallback:") {
        return .init(title: "Suggested fallback profile", detail: detail)
      }
      if let detail = suffix(in: cleaned, after: "Stream timing config:") {
        return .init(title: "Applied stream timing profile", detail: detail)
      }
      ...
      if normalized.contains("disconnect requested") {
        return .init(title: "Received disconnect stream request", detail: cleaned)
      }
      if normalized.contains("stream stop took") {
        return .init(title: "Stream stop completed", detail: cleaned)
      }
    ...
    case "input.scroll":
      if normalized.contains("scroll-trace start") {
        return .init(title: "Wheel input started", detail: cleaned)
      }
      if normalized.contains("scroll-trace render") {
        return .init(title: "Wheel effect displayed", detail: cleaned)
      }
      return .init(title: cleaned, detail: nil)
    ...
    case "ui.window":
      if normalized.contains("startup display mode requesting fullscreen toggle") {
        return .init(title: "Requested fullscreen switch", detail: nil)
      }
  ...
  private static func friendlyNetworkFailureTitle(for code: String) -> String {
    switch code {
    case "-1202":
      return "Certificate verification failed; preparing fallback"
    case "-1200":
      return "TLS secure connection failed; preparing fallback"
    case "-1001":
      return "Request timed out; preparing fallback"
    case "-1004":
      return "Could not connect to server; preparing fallback"
    case "-1005":
