    ...
    private func normalizedAuthorizationError(_ message: String) -> String {
        ...
        guard !trimmedMessage.isEmpty else {
            return isSandboxedBuild
                ? "No administrator authorization result was received. This build's security restrictions may have blocked the system authorization prompt."
                : "No administrator authorization result was received. Please try again."
        }

        if trimmedMessage.contains("(-128)") {
            return "You cancelled administrator authorization."
        }
        ...
        if trimmedMessage.contains("(-60005)") {
            return isSandboxedBuild
                ? "The system did not show the administrator authorization window correctly. This build's security restrictions may have blocked this type of request."
                : "Administrator authorization did not complete. Confirm that the current account has administrator privileges, then try again."
        }
